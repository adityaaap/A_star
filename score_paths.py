...
import numpy as np
import yaml
from nest_info import DeliverySite, NestInfo
...
def is_keepout_valid(nest_info: NestInfo, path: typing.List["tuple"]) -> bool:
    ...
    """
    return all(
        [nest_info.risk_zones[int(coord[0])][int(coord[1])] < NestInfo.KEEP_OUT_VALUE for coord in path]
    )
...
def get_results(nest_info: NestInfo, delivery_sites_with_paths: typing.List["DeliverySite"]) -> str:
    results_dict = dict()
    for site in delivery_sites_with_paths:
        ...
        if is_keepin_valid(nest_info, site.path):
            total_risk = sum(
                nest_info.risk_zones[int(coord[0])][int(coord[1])] == NestInfo.HIGH_RISK_VALUE
...
def display_and_save_result_image(nest_info: NestInfo, delivery_sites: typing.List["DeliverySite"]) -> None:
    fig = plt.figure()
